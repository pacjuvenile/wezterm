...
fn build_windows_resources() {
    ...
    let repo_dir = std::env::current_dir()
        ...
        .unwrap();
    let exe_output_dir = cargo_output_dir(&repo_dir, &profile);
...
fn copy_macos_plist() {
    ...
    let src_plist = repo_dir
        ...
        .join("Contents")
        .join("Info.plist");
    let build_target_dir = cargo_output_dir(&repo_dir, &profile);
...
fn cargo_output_dir(repo_dir: &std::path::Path, profile: &str) -> std::path::PathBuf {
    let mut output_dir = std::env::var_os("CARGO_TARGET_DIR")
